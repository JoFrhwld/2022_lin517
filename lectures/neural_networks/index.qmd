...
The output layer of the neural network is going to have 5 nodes, one for each vowel, and the values in each node will be the probability that the vowel it's classifying belongs to the specific vowel class. Let's say the specific token was [[Face]{.smallcaps}]{style="font-variant: small-caps;"} vowel. The output and the *desired* output might look like this:
